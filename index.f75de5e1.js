"use strict";
/* global gsap */ document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function updateLeftArrowFilter(index) {
        if (index > 0) leftArrow.classList.add("features__buttons--isActive");
        else leftArrow.classList.remove("features__buttons--isActive");
    }
    function updateRightArrowFilter(index) {
        if (index === totalSlides - 1) rightArrow.classList.add("features__buttons--isDisabled");
        else rightArrow.classList.remove("features__buttons--isDisabled");
    }
    showSlide(currentSlide);
...
lanLinks.forEach((link)=>{
    ...
    if (currentPage.includes(linkHref)) link.classList.add("menu__lan--link-isActive");
});
hederLanLinks.forEach((link)=>{
    const linkHref = link.getAttribute("href");
    if (currentPage.includes(linkHref)) link.classList.add("header__nav--link-isActive");
});
gsap.fromTo(".header__contant", {
    opacity: 0,
    x: -100
}, {
    opacity: 1,
...
gsap.fromTo(".about__title--mobile", {
    opacity: 0,
    x: -100
}, {
    opacity: 1,
...
gsap.fromTo(".about__content--first", {
    opacity: 0,
    x: 100
}, {
    opacity: 1,
...
gsap.fromTo(".about__picture--first", {
    opacity: 0,
    x: -100
}, {
    opacity: 1,
...
gsap.fromTo(".about__content--second", {
    opacity: 0,
    x: -100
}, {
    opacity: 1,
...
gsap.fromTo(".about__picture--second", {
    opacity: 0,
    x: 100
}, {
    opacity: 1,
...
gsap.fromTo(".contacts__container ", {
    opacity: 0,
    x: 100
}, {
    opacity: 1,
